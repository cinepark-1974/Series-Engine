...
★ v2.0.9 미세 패치 (2026-05-31)
  [패치 N] 비트 간 장면 반복 차단 강화 (prompt.py)
    · 원인: 「범죄스릴러」 EP1 실측에서 같은 EP 안의 장면이 반복 생성됨.
            - 덕문 사무실 보고 장면(블라인드/물컵/부하 보고/'누가 썼어')이
              Beat 0·1·2 등 여러 비트에 거의 동일하게 재생성.
            - 회의실 상부 보류 장면(상급 간부 대사)이 Beat 1·2에 토씨까지 복제.
            두 가지 구조적 결함:
            (1) prompt.py build_write_episode_beat_prompt의 중복 차단 블록이
                `if beat_num > 1`로 시작 → Beat 0·1을 건너뛰어, Beat 0이 그린
                장면이 Beat 1 차단 목록에 들어가지 않음.
            (2) 차단 정보가 직전 비트 "메모 첫 줄(추상 요약)"뿐 → 모델이
                구체적 장면 구성요소(장소·구도·대사)를 인지 못 해 같은 셋업 재생성.
    · 수정:
            (1) `beat_num > 1` → `beat_num >= 1`. Beat 1부터 직전 비트의
                장면이 차단 목록에 포함.
            (2) _extract_scene_signatures() 헬퍼 신규 추가. 각 비트 본문에서
                씬별 (장소 / 시간대 / 핵심 대사 1개)를 추출해 전달 →
                "같은 장소+같은 인물+같은 대사 패턴" 조합 자체를 차단.
            (3) 차단 지시문 강화 — 보고·지시·통보 셋업 반복 명시 금지.
    · 효과: 같은 EP 안에서 덕문 사무실/회의실 보고 장면의 반복 복제 차단.
            실제 백업 JSON 본문 구조로 추출 로직 검증 완료.

  [패치 O] 내부 메모 본문 누수 차단 (main.py)
    · 원인: 비트 본문 뒤에 붙는 [내부 메모] 진단 블록(비트 요약·SCOPE 검증·
            5단계 추적·AI ESCAPE 점검·장르 강제 체크·빌런 추적·보이스 점검 등)이
            최종 모드 DOCX 본문에 새어나옴. 기존 _strip_prop_state_memos는
            [소품 상태]·INTERNAL·<*_CHECK> 태그만 제거하고 [내부 메모] 블록
            본체는 통과시켰음.
    · 수정: _strip_prop_state_memos 최상단에 [내부 메모]/**내부 메모** 헤더부터
            텍스트 끝까지를 통째로 잘라내는 패턴 추가. 본문 내 정상 구분선
            (--- / --- TITLE ---)은 헤더 앞에서 끝나므로 보존됨.
    · 효과: EP1 8비트 전수 검증 — 메타 키워드 100% 제거, 본문 온전 보존.
"""
...
ENGINE_VERSION = "v2.0.9"
ENGINE_BUILD_DATE = "2026-05-31"
...
def _strip_prop_state_memos(text: str) -> str:
    ...
    # ★ v2.0.9 — [내부 메모] / **내부 메모** 블록 통째 제거
    # 비트 본문 뒤에 붙는 진단 메모(비트 요약·SCOPE 검증·5단계 추적·
    # AI ESCAPE 점검·장르 강제 체크·빌런 추적·보이스 점검 등)가
    # 최종 모드 DOCX 본문에 새어나오는 문제 차단.
    # 헤더가 나타나는 지점부터 텍스트 끝까지를 잘라낸다. 본문 내부의
    # 정상 구분선(--- / --- TITLE ---)은 헤더 앞에서 끝나므로 보존된다.
    pattern_internal_memo = _re_prop.compile(
        r'\n*(?:-{3,}\s*\n+)?\s*(?:\[내부\s*메모\]|\*\*내부\s*메모\*\*)[\s\S]*$',
        _re_prop.IGNORECASE,
    )
    text = pattern_internal_memo.sub('', text).rstrip()
