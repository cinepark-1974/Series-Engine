"""
👖 BLUE JEANS SERIES ENGINE v2.1.0 — main.py
...
★ v2.1.0 미세 패치 (2026-06-01)
  [패치 N] 세계관/스케일 비트 주입 — 글로벌 스케일 축소 차단
    · 원인: 「왕게임」 EP1 각본 실측에서, 기획안이 약속한 글로벌 스케일
            (삼각 경쟁 빌런 · 국제 물류망 · 박철 네 번째 축)이 각본에서
            무안군·서창서·인천항 단위의 로컬 사건으로 축소됨.
            데이터(inputs.world 1940자)와 season_arc는 글로벌 스케일을
            정확히 담고 있으나, 비트 집필 함수가 이를 받지 못하는
            구조적 결함이 있었음:
            - build_write_episode_beat_prompt는 로그라인·시즌아크 요약·
              씬 플랜·캐릭터 바이블·톤·트리트먼트·story_elements를 받지만
              inputs.world(세계관 전문)를 받는 인자 자체가 없었음.
            - 결과: 모델이 로컬 씬 플랜만으로 집필 → 세계의 규모가
              동네 단위로 축소되고, 다중 빌런(하사르·하상)이 단서로만
              암시되어 삼파전 구조가 EP 단위에서 작동하지 않음.
    · 수정:
            (1) build_write_episode_beat_prompt에 world_setting 인자 추가
                (기본 "", 하위 호환).
            (2) 세계관 전문을 [🌐 세계관 — 스케일·규칙·권력구조] 블록으로
                주입 (최대 2500자, 잘림 최소화).
            (3) 스케일 환기 강제 — 로컬 장면에도 더 큰 네트워크를
                환기시키는 디테일 최소 1개 강제.
            (4) 다중 빌런 현존 강제 — 빌런 2인 이상이면 각 빌런을
                흔적·대리인·영향력으로 EP당 최소 1회 현존.
            (5) main.py 호출부에서 inputs.world를 world_setting으로 전달.
    · 효과: 비트 집필 시 세계의 규모·규칙·삼각 권력구조가 항상 컨텍스트에
            포함됨. 로컬 사건이 글로벌 네트워크의 일부로 그려지고,
            등장하지 않는 빌런도 EP 단위로 존재감을 유지.

  [패치 O] A-Story 핵심 사건 장면화 강제 — 도입 사건 누락 차단
    · 원인: 「왕게임」 EP1에서 시즌 아크 A-Story에 명시된 '피의 밤'
            (마약왕 3명 동시 사망 + 증거 창고 화재)이 각본에 장면으로
            생성되지 않음. 단어로만 대사·메모에 참조되고 사건 자체는
            그려지지 않음. 손실 지점이 비트 집필이 아니라 한 단계 앞인
            씬 플랜 생성 단계였음:
            - 시즌 아크 EP1은 피의 밤을 A-Story 60% 핵심으로 명시했으나,
              씬 플랜(build_episode_plan_prompt)에 "아크 A-Story의 핵심
              사건을 빠짐없이 장면화하라"는 강제 규칙이 없었음.
            - 씬 플랜이 피의 밤을 전사(前史)로 요약 처리 → 비트 집필은
              씬 플랜만 따르므로 장면이 생성되지 않음.
            - 추가로 season_arc[:4000] 잘림으로 후반 EP의 A-Story가
              프롬프트에서 사라질 위험도 상존.
    · 수정:
            (1) build_episode_plan_prompt에서 해당 EP 구간을 시즌 아크에서
                "### EP{n}" 헤더 기준으로 추출해 별도 전문 블록으로 전달
                (잘림 보완).
            (2) [A-Story 핵심 사건 장면화 강제] 블록 추가 — 아크 A-Story의
                사건을 전부 씬으로 분해. 요약·회상 대체 금지.
            (3) 도입부 충격 사건(판을 여는 사건)은 Cold Opening 또는
                Beat 1에 실시간 장면으로 배치 강제. 전사 처리 금지.
            (4) 출력 형식 자가 검증에 'A-Story 핵심 사건 → S#번호 대응
                점검' 항목 추가.
    · 효과: 시즌 아크가 약속한 핵심 사건이 씬 플랜에서 누락되지 않음.
            피의 밤 같은 도입 사건이 장면으로 분해되어 각본에 생성됨.
"""
        ...
        if next_beat is not None:
            ...
            if st.button(
                ...
                prompt = build_write_episode_beat_prompt(
                    ...
                    # ★ v2.0.9 — 세계관/스케일 전문 주입 (글로벌 스케일 축소 방지)
                    world_setting=st.session_state.get("inputs", {}).get("world", ""),
                )
